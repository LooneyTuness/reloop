-- SQL script to create comprehensive sold items tracking system
-- This creates tables to track sold out items by vendor for better inventory management
-- Run this in your Supabase SQL Editor

-- 1. Create sold_items table to track all sold items
...
CREATE TABLE IF NOT EXISTS inventory_tracking (
  ...
  -- Status tracking
  is_sold_out BOOLEAN GENERATED ALWAYS AS (available_stock <= 0) STORED,
  ...
  IF NEW.status = 'completed' AND (OLD.status IS NULL OR OLD.status != 'completed') THEN
    ...
    SET 
      current_stock = GREATEST(0, current_stock - (
        ...
        FROM order_items oi 
        WHERE oi.item_id = inventory_tracking.item_id 
...
GRANT SELECT, UPDATE ON inventory_tracking TO authenticated;
...
-- Success message
SELECT 'Sold items tracking system created successfully!' as message;
